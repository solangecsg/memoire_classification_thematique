...
Solange Cussaguet (alternance dans le cadre du Master TNAH, École nationale des chartes, BnF-Partenariats).

25 juillet 2026

**Fraîcheur des tarifs** : tous les tarifs cités dans ce document (API Mistral, location GPU) ont été vérifiés à la date de rédaction ci-dessus. Ils sont susceptibles d'avoir changé depuis et sont à revérifier avant toute réutilisation (sources listées section 9).
...
- **Approche recommandée** : **groupage par lot** (`classify_iptc_mistral_batched.py`) , soit regrouper plusieurs articles par appel plutôt qu'un seul, pour partager le coût fixe de la liste d'étiquettes. C'est de loin le levier le plus rentable (-89 % vs 1 article/appel), largement testé et documenté (sections 4 à 6).
- **Ce qui a été essayé et écarté** : découper la classification en 2 étages (niveau 2 puis niveau 3 par branche) pour réduire encore la taille de la liste. Cela coûte en réalité plus cher (section 8). Gardé dans le dépôt à titre d'exemple documenté.
- **Budget** : ~$3,63 pour l'échantillon de 100 fascicules (6699 articles, mistral-large-latest, groupage, sans cache) ; extrapolé linéairement à ~$94 000 pour 2,6M fascicules (section 6). L'extrapolation est à prendre comme ordre de grandeur, pas une prévision ferme.
- **Hébergement local (GPU cloud)** : c'est l'hypothèse la moins chère de tout ce document, environ $2 570 à $4 890 pour 2,6M fascicules (section 7), contre ~$94 380 pour le cloud groupé avec mistral-large (section 6), soit environ 20 à 35 fois moins cher. **Attention** : ce chiffre est une projection basée sur des benchmarks GPU publiés sur d'autres prompts, pas une mesure réelle sur ce prompt exact. Le protocole de vérification (section 11) n'a pas encore été exécuté (voir aussi "Questions ouvertes").

## Ce qui a été livré

- **4 scripts de classification IPTC** : référence 1 article/appel, variante avec cache de prompt, groupage par lot (retenu), cascade 2 niveaux (testée et écartée). Les 4 sont fonctionnels, testés sur le corpus échantillon, avec suivi des tokens/coût/temps par appel et reprise automatique sur interruption.
- **1 pipeline de ré-OCRisation en 2 variantes** : Mistral vision (`pixtral-12b`) et Pero, avec scripts et notebooks Colab, appliqué à 100 fascicules.
- **Des outils de mesure de qualité OCR** : comparaison BnF original / Pero / Mistral (statistiques de vocabulaire, score impresso OCR Quality Assessment).
- **4 scripts d'estimation de coût sans appel API** : statistiques du corpus, coût par scénario, comparaison entre modèles, distribution réelle des branches pour la cascade.
- **Une application Streamlit d'annotation** pour produire une vérité terrain et comparer visuellement les résultats du groupage par lot au mode 1 article/appel (reste en local, non incluse dans ce dépôt).
- **Une recherche documentée sur l'hébergement local** : benchmarks GPU réels (vLLM, RTX 4090), comparaison de licences de modèles (Ministral Research License vs Qwen3 Apache 2.0), protocole de test prêt à exécuter (section 11).
- **Ce document de synthèse**, qui consolide l'ensemble des mesures et décisions prises pendant l'alternance.

## ⚠️ Risque principal non résolu : la qualité de classification n'a jamais été validée

**Aucune vérité terrain n'existe pour ce projet.** Tous les chiffres de ce document mesurent le **coût** et le **volume** (tokens, appels, temps) : pas la **justesse** des thèmes assignés par le modèle. Une interface de relecture humaine existe (`app_annotation_batched.py`, Streamlit, en local ; voir le dépôt de travail, pas inclus dans ce GitHub pour l'instant) mais n'a été utilisée que pour explorer manuellement quelques articles, jamais pour produire un échantillon de vérité terrain statistiquement exploitable.

Le détail de ce qui manque concrètement avant tout déploiement à l'échelle est regroupé dans la section "Questions ouvertes" ci-dessous.
...
- **Mémoire et ressources complémentaires** : un dépôt séparé, public, sera créé à l'occasion de la soutenance du mémoire, avec des ressources complémentaires (bibliographie, méthodologie détaillée, etc.). 
- **Contact après la fin de l'alternance** : Solange Cussaguet : s.cussaguet@gmail.com et Olivier Garre : olivier.garre@bnf.fr.

### Documentation externe citée dans ce document

- **API Mistral** : docs.mistral.ai (documentation générale, endpoints, structured outputs) et mistral.ai/pricing/api (tarifs par modèle, à revérifier avant réutilisation, voir avertissement en en-tête).
- **Taxonomie IPTC Media Topics** : cv.iptc.org/newscodes/mediatopic (source du fichier `iptc_mediatopic_official.json`, format SKOS/JSON-LD, licence CC BY 4.0).
- **vLLM (hébergement local)** : docs.vllm.ai (serveur d'inférence, structured outputs, prefix caching, voir section 11.2).

## Conformité et licences

- **Données Gallica/BnF (corpus source)** : les conditions de réutilisation du corpus texte (ALTO/METS) dépendent des conditions générales d'utilisation de Gallica. Non vérifiées en détail dans le cadre de ce projet : à valider avec le service Gallica/le service juridique de la BnF avant toute réutilisation hors du contexte de ce POC, en particulier pour une diffusion publique du corpus lui-même (voir aussi la note sur les images, volontairement exclues de ce dépôt).
- **Taxonomie IPTC Media Topics** : licence CC BY 4.0, vérifiée à la source (cv.iptc.org/newscodes/mediatopic) au moment de la récupération du fichier `iptc_mediatopic_official.json`.
- **API Mistral** : usage soumis aux conditions générales d'utilisation de Mistral AI (docs.mistral.ai), non reproduites ici, à consulter directement avant tout usage en production. Le corpus contient du texte de presse ancienne, a priori dans le domaine public compte tenu de son ancienneté, mais cela n'a pas été vérifié formellement titre par titre.
- **Modèles alternatifs évalués pour l'hébergement local** : Ministral 8B sous Mistral Research License (usage non commercial uniquement) ; Qwen3-8B sous licence Apache 2.0 (usage commercial autorisé). Détail en section 11.1.

## Questions ouvertes

Ce qui reste à vérifier avant de considérer les résultats de ce document comme fiables pour une mise en production, regroupé ici en un seul endroit :

- **Qualité de classification** : aucune vérité terrain n'existe, la justesse des thèmes assignés par le modèle n'a jamais été mesurée.
- **Effet du groupage par lot sur la qualité** : le risque théorique de dégradation avec plusieurs articles par appel ("lost in the middle") n'a jamais été testé sur ce prompt précis.
- **Comparaison de qualité entre modèles** : rien ne garantit qu'un modèle moins cher (Ministral 8B/3B, section 5) classe aussi bien que mistral-large.
- **Représentativité de l'échantillon** : les 100 fascicules mesurés viennent d'un seul titre et d'une seule période ; l'extrapolation à 2,6M fascicules (section 6) suppose une composition similaire pour tout le corpus, non vérifiée.
- **Taux de cache réel en mode groupage** : calibré uniquement en mode 1 article/appel (section 3, note), jamais mesuré sur un run réel en groupage.
- **Débit GPU réel sur ce prompt exact** : les chiffres de la section 7 viennent de benchmarks publiés sur d'autres prompts (préfixe court, sortie longue), pas d'une mesure sur notre profil (préfixe énorme, sortie minuscule). Protocole de vérification prêt en section 11, pas encore exécuté.
- **Prix de location GPU** : ordre de grandeur observé (environ $0,29 à $0,59/h), pas un devis ou contrat réel.
...
**Précision sur le "prompt caching"** : le cache à -90 % des sections 4 à 6 est une fonctionnalité de facturation propre à l'API cloud Mistral, elle ne s'applique pas ici puisque l'hébergement local n'appelle plus l'API Mistral. Son équivalent fonctionnel côté local est le **cache de préfixe automatique de vLLM** (`--enable-prefix-caching`, section 11.2) : gratuit plutôt qu'à -90 %, et **déjà activé par défaut dans tous les chiffres ci-dessous**, pour les deux scénarios (1 article/appel et groupage par lot). Le scénario "groupage par lot" de cette section correspond donc déjà à la combinaison groupage + cache de préfixe + GPU cloud.
...
### Récapitulatif : l'hébergement local est l'hypothèse la moins chère de ce document

| Scénario (2,6M fascicules) | Coût estimé | Statut |
|---|---:|---|
| 1 article/appel, cloud (mistral-large, sans cache) | $829 660 | Extrapolé linéairement à partir de l'échantillon mesuré (section 6) |
| Groupage par lot, cloud (mistral-large, sans cache) | $94 380 | Extrapolé linéairement à partir de l'échantillon mesuré (section 6) |
| Groupage par lot, cloud (ministral-3b, sans cache) | $17 160 | Idem, modèle moins cher (section 6) |
| **Hébergement local (GPU cloud, débit 2 259 à 4 173 tok/s)** | **$2 570 à $4 890** | **Projection : débit issu de benchmarks publiés sur d'autres prompts, prix GPU un ordre de grandeur observé, pas mesuré sur ce prompt exact (voir section 11 et "Questions ouvertes")** |

L'hébergement local ressort comme le scénario le moins cher de toutes les hypothèses testées dans ce document, environ 20 fois moins cher que le cloud groupé avec mistral-large. C'est cependant la seule ligne de ce tableau qui n'est pas ancrée dans une mesure réelle sur notre corpus : le débit GPU et le prix de location sont des estimations externes (sourcées, mais pas mesurées sur ce prompt, voir section 9), à vérifier avant de fonder une décision dessus (protocole de test en section 11).
...
**Résultat : la cascade coûte plus cher que le groupage simple**
...
**Conclusion** : l'idée était logique sur le papier (la liste dominait le coût), mais elle ignorait le coût de la duplication du texte. Elle ne vaudrait le coup que pour des articles très courts par rapport à la taille de la liste, à l'inverse de notre corpus (522 tokens/article en moyenne, contre une liste déjà réduite à ~2000 tokens à l'étage 1).
...
1. **Le groupage par lot est le levier le plus rentable en cloud** (-89 % à lui seul) : largement devant le choix d'un modèle moins cher, ou la cascade niveau 2 → niveau 3 (qui coûte plus cher, voir section 8) : et sans compromis connu sur la fenêtre de contexte (marge x5 même dans le pire cas).
...
## 12. Feuille de route pour un éventuel déploiement

Ordre d'action recommandé pour une équipe qui reprendrait ce travail :

1. **Vérifier que le corpus cible dispose du découpage à l'article** (structure METS/TOC avec divs `ARTICLE`, voir avertissement en introduction et section 1). Si ce n'est pas le cas, c'est un prérequis avant toute autre étape : ce POC ne fonctionne pas sans ce découpage.
2. **Constituer un échantillon de vérité terrain** (quelques centaines d'articles annotés à la main) avant tout déploiement à l'échelle. C'est le risque non résolu le plus important du projet (voir "Questions ouvertes") : sans ça, aucune des recommandations de coût de ce document ne garantit une classification fiable.
3. **Choisir entre cloud et hébergement local** selon la priorité réelle : le cloud groupé (`classify_iptc_mistral_batched.py`, mistral-large ou un modèle moins cher) reste le plus simple à mettre en œuvre. L'hébergement local ne devient pertinent que si la confidentialité ou le contrôle du modèle priment sur la simplicité (sections 7 et 11).
4. **Si l'hébergement local est retenu**, exécuter le protocole de test réel décrit en section 11 (GPU cloud + vLLM, sur un petit échantillon) avant de se fier aux estimations basées sur des benchmarks publiés.
5. **Trancher la question de la conformité** (réutilisation des données Gallica/BnF, licence de la taxonomie IPTC, conditions d'usage de l'API du modèle choisi) avant toute mise en production, voir "Conformité et licences".
6. **Ne pas reprendre la piste cascade** (section 8) : déjà testée et mesurée comme plus coûteuse que le groupage simple, sauf si un futur corpus a des articles nettement plus courts que celui testé ici (voir la limite identifiée en section 8).
